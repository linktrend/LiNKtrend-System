...
Current repo ownership and completion targets are defined in `docs/architecture/repo-architecture-target.md` and `docs/architecture/system-completion-targets.md`. Older references in this narrative to vertical plugins and capability plugins should be read as modules and capability connectors.
...
The risk is that raw connectivity is becoming commoditized through MCP, OpenAPI, and ecosystem connectors. The value of capability plugins is not merely “we connect to QuickBooks.” The value is that the connection is governed, tenant-scoped, typed, auditable, swappable, and usable by LiNKbot and workflows under policy.
...
The admin route controls tenant settings, plugin installs, capability choices, credentials, cost caps, and kill switches. The company route shows human and bot org structure. The workers route manages LiNKbot. The work route shows active work. The projects route surfaces engagements across verticals. The skills route reads LinkSkills. The memory route reads LiNKbrain. The traces route shows audit. The metrics route shows cost, performance, automation savings, and operational pulse. The gateway route shows inbound integration events.
...
- `LiNKaios/linkaios-web` for the Next.js command center.
...
- `modules/` for WebsiteFactory, VentureStudio, MediaProduction, Ecommerce, LawFirm, and future tenant-enabled modules.
- `LiNKskills/capability-connectors/` for accounting, CRM, document management, messaging, project management, analytics, marketing, email, storage, payments, and other horizontal capability connectors.
