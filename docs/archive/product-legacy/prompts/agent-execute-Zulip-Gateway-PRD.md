# Agent prompt — Execute `../pre-ecosystem/Zulip-Gateway-PRD.md` (v1 gateway ready)
...
Single source of truth: **`../pre-ecosystem/Zulip-Gateway-PRD.md`**.  
Read **`LiNKbot/communications/temporary-gateways/zulip/src/index.ts`**, **`zulip-payload.ts`**, **`zulip-send.ts`**, **`../pre-ecosystem/zulip-routing.md`**, and **`packages/shared-config/src/index.ts`**.
...
2. **Inbound webhook hardening:** Max body bytes (reject early). Optional **`ZULIP_INBOUND_WEBHOOK_SECRET`**: if set, require match on agreed header **or** query param; document in `../pre-ecosystem/zulip-routing.md`. Add fields to **shared-config** schema.
...
5. **Docs:** Update **`../pre-ecosystem/zulip-routing.md`** with resolution order, env table, and “configure Zulip outgoing webhook” checklist including optional secret.
