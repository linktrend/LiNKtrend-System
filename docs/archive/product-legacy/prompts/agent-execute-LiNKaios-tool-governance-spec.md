# Agent prompt — Execute `../pre-ecosystem/LiNKaios-tool-governance-spec.md` only
...
Read and follow **`../pre-ecosystem/LiNKaios-tool-governance-spec.md`** end-to-end. Do **not** implement LiNKskills catalog authoring for `declared_tools` or SKILL.md editor UX — that is **`../pre-ecosystem/LiNKskills-PRD.md`** (another agent). Assume skills may expose `declared_tools` in metadata or governance payload once the other track lands; until then, use **manifest allowlists** and existing skill resolution without breaking builds.
...
2. **RLS:** Policies consistent with `command_centre_write_allowed()`; extend with role helpers for org admin vs project head as needed (see `../pre-ecosystem/operator-roles.md` if present).
...
6. **Zulip:** `LiNKbot/communications/temporary-gateways/zulip` (or agreed notifier): post message with **one-shot LiNKaios URL** on block/request; respect `../pre-ecosystem/zulip-routing.md` and `gateway.stream_routing`; define fallback for mission-less (spec §6.2).
