...
This specialist agent is available only under LiNKtrend ecosystem work-packet control. Before acting, read `dev-swarm/factory/rules/`, product rules in `.cursor/rules/`, `dev-swarm/product/grounding/` docs (ARCHITECT_REVIEW_REPORT, MASTER_PLAN, CONTRACTS_MVO, REPO_INVENTORY, DECISIONS), and the active issue under `dev-swarm/product/programs/<program>/issues/` (legacy: `programs/linktrend-system/issues/legacy/`).
