...
You are configuring **Cursor** automations for Dev Swarm. Use computer use to navigate the Cursor UI. Do not store secrets in the repo.
...
2. Name exactly as manifest (e.g. `dev-swarm-orchestrator`).
...
- Test label `swarm:review-ready` on a dry-run issue does not run until Reviewer automation exists

## Handoff

Report automation names created in `dev-swarm/product/reports/wire-automation-setup.md` (create if missing).
