...
BRANCH="${DEV_SWARM_INTEGRATION_BRANCH:-development}"
...
def state_candidates() -> list[Path]:
    return [
        root / "dev-swarm/factory/STATE.md",
        root / f"dev-swarm/product/programs/{program}/STATE.md",
    ]
...
def done_issues() -> list[str]:
    for path in state_candidates():
        ...
        if done:
            return sorted(done)
    plan = root / f"dev-swarm/product/programs/{program}/PROGRAM.md"
    if not plan.is_file():
        plan = root / f"dev-swarm/factory/programs/{program}/PROGRAM.md"
