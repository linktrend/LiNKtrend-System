#!/usr/bin/env bash
# Run Dev Swarm completion gates from factory/gates/catalog.json.
set -euo pipefail
...
CATALOG="$ROOT/dev-swarm/factory/gates/catalog.json"
...
Environment fallbacks: DEV_SWARM_PROGRAM, DEV_SWARM_REPORT, DEV_SWARM_SCOPE
EOF
  exit 2
}
...
PROGRAM="${PROGRAM:-${DEV_SWARM_PROGRAM:-}}"
REPORT="${REPORT:-${DEV_SWARM_REPORT:-}}"
...
STACK_FILE="dev-swarm/product/grounding/STACK.md"
SHIP_CRITERIA="dev-swarm/product/grounding/SHIP_CRITERIA.md"

program_plan() {
  if [[ -n "$PROGRAM" ]]; then
    local plan="dev-swarm/product/programs/${PROGRAM}/PROGRAM.md"
    [[ -f "$plan" ]] && { echo "$plan"; return; }
    plan="dev-swarm/factory/programs/${PROGRAM}/PROGRAM.md"
    [[ -f "$plan" ]] && { echo "$plan"; return; }
  fi
  echo "dev-swarm/factory/programs/bootstrap/PROGRAM.md"
}
...
release_report() {
  ...
  if [[ -n "$PROGRAM" ]]; then
    local status="dev-swarm/product/reports/${PROGRAM}/STATUS.md"
...
council_report_path() {
  local gate="$1"
  local path=""
  if [[ -n "${DEV_SWARM_COUNCIL_REPORT:-}" && -f "$DEV_SWARM_COUNCIL_REPORT" ]]; then
    echo "$DEV_SWARM_COUNCIL_REPORT"
    return
  fi
  [[ -n "$PROGRAM" ]] || return 1
  case "$gate" in
    G3)
      if [[ -n "${DEV_SWARM_MODULE:-}" && -n "${DEV_SWARM_PHASE:-}" ]]; then
        path="dev-swarm/product/reports/${PROGRAM}/${DEV_SWARM_MODULE}/${DEV_SWARM_PHASE}/council-report.json"
        [[ -f "$path" ]] && { echo "$path"; return; }
      fi
      path="$(find "dev-swarm/product/reports/${PROGRAM}" -name 'council-report.json' 2>/dev/null | head -1 || true)"
      [[ -n "$path" && -f "$path" ]] && { echo "$path"; return; }
      ;;
    G4)
      path="dev-swarm/product/reports/${PROGRAM}/council/G4-release-report.json"
      [[ -f "$path" ]] && { echo "$path"; return; }
      ;;
  esac
  return 1
...
find_issue_file() {
  ...
  done < <(find dev-swarm -name "${issue_id}.md" -path '*/issues/*' 2>/dev/null)
  return 1
}
...
for i, line in enumerate(text.splitlines(), 1):
    if re.match(r"^- \[ \]", line):
        if "Chairman Release OK" in line:
            continue
    ...
    script)
      ...
      case "$gate_id" in
        ...
        council_g4_report)
          ...
          ;;
        verify_subset)
          if DEV_SWARM_SCOPE="$SCOPE" DEV_SWARM_SKIP_GATES=1 "$ROOT/$script_rel"; then
            log_ok "$gate_id"
          else
            log_fail "$gate_id ($script_rel)"
          fi
          ...
          ;;
        verify_critical)
          if DEV_SWARM_SCOPE="." DEV_SWARM_TIER=critical DEV_SWARM_SKIP_GATES=1 "$ROOT/$script_rel"; then
            log_ok "$gate_id"
          else
            log_fail "$gate_id ($script_rel)"
          fi
...
echo "== Dev Swarm gates tier=$TIER scope=$SCOPE program=${PROGRAM:-—} report=${REPORT:-—} =="
