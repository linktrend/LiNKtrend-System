...
5. **G1 council** — after narrative OK, run council review against `dev-swarm/factory/laws/DEV_SWARM_LAWS.md`. Any verdict with severity **BLOCKER** stops progress until resolved or Chairman waives (LAW-07). Do not proceed to program files until G1 is clear.
6. Write `dev-swarm/product/grounding/VISION.md`, `SHIP_CRITERIA.md`, and **copy** `dev-swarm/factory/templates/INTENT.md` → `dev-swarm/product/grounding/INTENT.md` (fill; link VISION + SHIP_CRITERIA).
7. **Copy** `dev-swarm/factory/templates/program-plan.md` → `dev-swarm/product/programs/<program-id>/PROGRAM.md` (fill; do not invent structure).
...
10. Run `dev-swarm/factory/scripts/validate-dag.sh` on PROGRAM.md.
11. **G2 intent verdict** — before setting STATE `phase: running`:
    - Check all eight laws (LAW-01…LAW-08) and program requirements.
    - Write `dev-swarm/product/reports/<program-id>/intent-verdict.json` using `factory/templates/intent-verdict.json`.
    - Set `status: PASS` **only** if laws + requirements pass and `blockers` is empty; otherwise `FAIL` or `BLOCKED` with blockers listed.
    - Run `dev-swarm/factory/scripts/validate-intent.sh <program-id>` — must exit 0 before handoff.
12. Hand off: STATE prepared for Orchestrator (`phase: running`) **only after G2 PASS**. Orchestrator must not set `swarm:ready` until intent verdict PASS and `validate-intent.sh` exits 0 — do not wait for second Go once PASS.
...
- Do not set `swarm:ready` — Orchestrator does after program exists and G2 intent verdict PASS.
