...
Multi-advisor gate review for Dev Swarm programs. Default runtime: **Cursor cloud** (Planner/Orchestrator handoff) or **Cursor automation** at phase boundaries.
...
|------|------|----------------------|------------|
| **G1** | After finished-product narrative; **before** `PROGRAM.md` | Chairman-approved finished-product narrative, draft `VISION.md` / `SHIP_CRITERIA.md` | Combined summary **PASS** or **WARN** (no BLOCKER) |
...
| **G4** | **Program release** (before Chairman Release OK / staging) | Full program STATUS, demo evidence, `SHIP_CRITERIA.md`, proof manifest | Combined summary **PASS** only (WARN requires Chairman ack) |
...
4. Merge persona outputs into `dev-swarm/factory/templates/council-SUMMARY.md` format.
5. Write machine-readable report JSON conforming to `dev-swarm/factory/contracts/council-report.schema.json`.
6. Run `dev-swarm/factory/scripts/validate-council.sh <report.json> [--gate Gn] [--allow-warn]`.
...
- Markdown summary from `dev-swarm/factory/templates/council-SUMMARY.md`
- JSON report at path chosen by triggering role, e.g.:
  - G1–G2: `dev-swarm/product/reports/<program-id>/council/G1-report.json`
  - G3: `dev-swarm/product/reports/<program-id>/<module-id>/<phase-id>/council-report.json`
  - G4: `dev-swarm/product/reports/<program-id>/council/G4-release-report.json`

## Triggers

| Gate | Triggered by |
|------|--------------|
| G1 | Planner after Chairman OK on finished-product narrative |
| G2 | Planner after `PROGRAM.md` + issue tree drafted |
| G3 | Orchestrator when all issues in a module phase are `done` |
| G4 | Integrator when program DoD checklist complete, before Chairman Release OK |
...
See `dev-swarm/factory/install/SKILLS-ALLOWLIST.md` → Council (Planner, Orchestrator, Integrator contexts).
