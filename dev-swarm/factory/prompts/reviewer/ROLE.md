...
```bash
export DEV_SWARM_TIER=<issue tier>
export DEV_SWARM_REPORT=<report_path>
dev-swarm/factory/scripts/run-gates.sh --tier A --report "$DEV_SWARM_REPORT"
```
...
| Fail | Remove merge-ready if set; comment gaps (include tier A gate failures); recommend `swarm:blocked` or return to executor |
