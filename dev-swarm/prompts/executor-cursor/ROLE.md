...
2. Read issue spec + `dev-swarm/SPEC.md`
...
2. `dev-swarm/scripts/verify.sh` (set `DEV_SWARM_TIER` from issue tier)
3. Update report with proof block (DS-B5)
4. Optional: `dev-swarm/scripts/proof-manifest.sh <report>`
5. Open PR; label `swarm:review-ready`; only set `swarm:merge-ready` if verify passed (DS-B9)

## Skills

See `dev-swarm/install/SKILLS-ALLOWLIST.md` → Executor (Cursor).
