...
for s in dev-swarm/scripts/*.sh; do
  [[ -f "$s" ]] || continue
...
for j in dev-swarm/contracts/*.json; do
...
if [[ "$TIER" == "critical" ]]; then
  if ! dev-swarm/scripts/validate-dag.sh dev-swarm/programs/bootstrap/PROGRAM.md 2>/dev/null; then
