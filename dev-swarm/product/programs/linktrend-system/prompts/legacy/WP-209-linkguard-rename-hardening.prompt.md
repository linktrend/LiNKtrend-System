...
Execute `dev-swarm/product/programs/linktrend-system/issues/legacy/WP-209-linkguard-rename-hardening.md`.
...
You are the LiNKguard hardening agent. Verify package name, Dockerfile, compose service, residue policy tests, README/source references, and LiNKaios settings labels so no active code still depends on PRISM/prism-defender paths. Update `dev-swarm/product/reports/archive/legacy-ai-swarm/WP-209-linkguard-rename-hardening.md` with files changed, commands run, proof, blockers, and next step.
