...
- `dev-swarm/product/grounding/CONTRACTS_MVO.md` — added §0.A LinkSites v2 canonical section (flow, hard boundaries, production artifact direction, canonical LiNKbot roles, required v1 capability plugins, site identity, side-effect routing, schema-related deferrals, mode model, acceptance posture, and v1 §§1–13 historical-relationship statement). Marked top-of-file status and v1 §§1–13 as historical reference.
- `dev-swarm/product/grounding/LINKAIOS_KERNEL_MANIFEST.md` — added §0.A v2 addendum (v2 work-request and stages, disabled LiNKbot roles for Lead Scout and Outreach, required capabilities, deferred concrete manifest pending WP-042, kernel non-ownership reaffirmed under v2, trace/status surface obligations). Marked §4 WebsiteFactory manifest as historical v1 reference.
- `dev-swarm/product/grounding/INTEGRATION_QUEUE.md` — added "LinkSites v2 capability integrations" section (INT-040..INT-049) plus explicit "out of scope for v2" list. Marked v1 stubbed-integrations section as historical.
- `dev-swarm/product/grounding/AGENT_COORDINATION.md` — appended WP-041 entry to Latest Updates.
- `dev-swarm/product/reports/archive/legacy-ai-swarm/linkaios-agent.md` — this entry.
- `dev-swarm/product/reports/archive/legacy-ai-swarm/integration-agent.md` — WP-041 cross-reference entry for v2 integration rows.
...
- Added `dev-swarm/product/grounding/LINKSITES_TEMPLATE_PAYLOAD_DISCOVERY.md` with facts, assumptions, blockers, and command-level proof.
