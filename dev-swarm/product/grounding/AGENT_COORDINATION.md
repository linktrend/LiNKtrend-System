...
**LinkSites / WebsiteFactory module in development mode on the reusable LiNKtrend platform slice.** The current MVO target is mock CRM lead → LiNKbot research/content/site build → local generated artifacts → Supabase mirror → LiNKautowork sync to local Payload CMS → preview-ready frontend → deterministic checks → CRM status `ready_to_contact`. Real lead acquisition, real client outreach, and real VPS deployment are disabled for the MVO.

Post-cleanup source-of-truth docs:

- `docs/architecture/repo-architecture-target.md`
- `docs/architecture/system-completion-targets.md`

Older rows in this file are historical progress logs and may use legacy `vertical plugin` / `capability plugin` wording.
...
| WP-046 | `WORK_PACKETS/WP-046-linksites-v2-sdk-contracts.md` | LinkSites v2 shared SDK contract types |
| WP-047 | `WORK_PACKETS/WP-047-linkskills-linksites-capability-catalog.md` | LinkSkills catalog entries for LinkSites v2 capabilities |
| WP-048 | `WORK_PACKETS/WP-048-linkautowork-linksites-workflow-scaffold.md` | LiNKautowork development-mode workflow scaffolds for LinkSites v2 |
| WP-049 | `WORK_PACKETS/WP-049-linksites-kernel-v2-stage-wiring.md` | LiNKaios kernel LinkSites v2 stage wiring |
| WP-050 | `WORK_PACKETS/WP-050-linkskills-v2-capability-execution-handlers.md` | LinkSkills mock/shadow execution handlers for v2 capabilities |
| WP-051 | `WORK_PACKETS/WP-051-kernel-autowork-v2-handle-integration.md` | Kernel to LiNKautowork v2 handle integration |
| WP-052 | `WORK_PACKETS/WP-052-linksites-v2-e2e-flow-harness.md` | LinkSites v2 E2E flow harness |
| WP-053 | `WORK_PACKETS/WP-053-zulip-communication-capability-scaffold.md` | Zulip communication capability scaffold |
| WP-054 | `WORK_PACKETS/WP-054-odoo-crm-accounting-capability-discovery-scaffold.md` | Odoo CRM and Accounting capability discovery scaffold |
| WP-055 | `WORK_PACKETS/WP-055-postiz-distribution-capability-scaffold.md` | Postiz distribution capability scaffold |
| WP-056 | `WORK_PACKETS/WP-056-lexos-vertical-repo-discovery.md` | LEXOS vertical repo discovery |
| WP-057 | `WORK_PACKETS/WP-057-linkapps-vertical-starter-kit-discovery.md` | LiNKapps starter-kit and workflow discovery |
| WP-058 | `WORK_PACKETS/WP-058-linkbrain-v2-audit-memory-coverage-review.md` | LiNKbrain v2 audit/memory coverage review |
| WP-059 | `WORK_PACKETS/WP-059-linkautowork-completion-plan-runtime-hardening.md` | LiNKautowork completion plan and hardening packets |
| WP-060 | `WORK_PACKETS/WP-060-linkskills-completion-plan-governance-service-hardening.md` | LinkSkills completion plan and governed skills service hardening |
| WP-061 | `WORK_PACKETS/WP-061-linkbot-core-upstream-sync-integration-readiness.md` | LiNKbot-core upstream sync and integration readiness |
| WP-062 | `WORK_PACKETS/WP-062-linkbot-linkaios-linkskills-zulip-adapter-plan.md` | LiNKbot adapter plan for LiNKaios, LinkSkills, and Zulip |
...
- **LiNKaios:** `LiNKtrend-System/LiNKaios/linkaios-web` (UI), `packages/linklogic-sdk` (SDK).
...
- **LiNKbot:** `LiNKbot-core` (external runtime), `LiNKtrend-System/LiNKbot/runtime-adapters/openclaw/bot-runtime` (adapter).
...
- 2026-05-15 — Expanded execution roadmap defined after LinkSites v2 scaffolding. Added WP-049..WP-062 covering LinkSites kernel/LinkSkills/LiNKautowork integration, v2 E2E proof, Zulip/Odoo/Postiz capability scaffolds, LEXOS and LiNKapps discovery, LiNKbrain coverage review, LiNKautowork and LinkSkills completion plans, and LiNKbot upstream sync/adapter planning. Prompt files are available under `dev-swarm/product/programs/linktrend-system/prompts/legacy/`. First larger launch wave should prioritize WP-049, WP-050, WP-051, WP-053, WP-054, WP-056, WP-057, and WP-061; WP-052 and WP-062 have dependency gates.
- 2026-05-15 — WP-042 and WP-043 integrated by Integrator. WP-042 confirmed `LiNKsites/apps/web-master` as the master template frontend, `marketing-smb-v1` as the currently registered template module, existing Payload CMS collections/config under `LiNKsites/apps/cms`, existing Supabase mirror clues under `LiNKsites/supabase`, and the frontend Payload reader path. WP-043 defined connector-only capability contracts for Odoo/CRM shadow-readiness, local Payload, Supabase mirror, Zulip messaging, public web research, asset generation, and Plane execution tracking. Integrator normalized WP-043 failure mappings to existing canonical `CONTRACTS_MVO.md` §5.4 codes before merge. Added WP-046..WP-048 implementation-scaffold packets and prompt files.
- 2026-05-15 — WP-040 completed by Architect. `CONTRACTS_MVO.md` and `LINKAIOS_KERNEL_MANIFEST.md` now make the plugin architecture v2 first-class: vertical vs capability plugin distinction (§1.0.1), `modes_supported` development/shadow/live (§1.0.2), LiNKbot role attachment model (§1.0.3), stop-and-ask review-gate (§1.0.4 + §12.7), and LinkSkills scoped as permissions + skills (§12.4). `PluginManifestSchema` in `@linktrend/linklogic-sdk` extended with optional `plugin_kind`, `modes_supported`, `required_linkbot_roles`, and a `capability` block (with cross-field validation); legacy v1 manifests still parse. Added decision row D-09. Verification: `pnpm --filter @linktrend/linklogic-sdk typecheck` clean; `pnpm --filter @linktrend/linklogic-sdk test -- contracts-mvo` → 11 files, 80 tests (38 contract tests including 7 new v2 manifest cases). Branch: `dev/codex/WP-040-plugin-architecture-contract-v2`.
- 2026-05-15 — WP-041 completed by Architect. LinkSites v2 development-mode flow is now the canonical contract target in source-of-truth docs. `CONTRACTS_MVO.md` gained §0.A pinning the v2 canonical flow, hard boundaries, production artifact direction (cloud cold storage, not live hosting), canonical LiNKbot roles (Lead Scout and Outreach declared but disabled), required v1 capability plugins, site identity (`site_id` + versioned `site_generation_run_id` unless WP-042 discovery contradicts), v2 side-effect routing through LinkSkills + LiNKautowork + LiNKbrain envelope, schema-related deferrals to WP-042, mode model (`development | shadow | live`), v2 acceptance posture, and explicit relationship to the historical v1 §§1–13. `LINKAIOS_KERNEL_MANIFEST.md` gained §0.A v2 addendum reaffirming kernel non-ownership and trace/status surface obligations under v2, and §4 is now flagged as historical v1 reference. `INTEGRATION_QUEUE.md` gained the "LinkSites v2 capability integrations" section (INT-040..INT-049) covering Odoo/CRM shadow-readiness, local Payload, Supabase mirror, Zulip, public web research, asset generation, Plane mock/shadow, local generated-artifact folder, deterministic checks, and frontend preview reader, with INT-020..INT-022 marked historical v1. No implementation code, no Payload/Supabase schemas, no VPS deploy path, no outreach send path was added. SDK contract files were intentionally not changed in WP-041; v2 wire-format types are deferred to a post-WP-042 packet so concrete schemas are not invented ahead of discovery.
...
- 2026-05-14 — WP-011 review found one blocker before WP-013: WebsiteFactory plugin exists with tests, but kernel execution still uses hardcoded mappings in `LiNKaios/linkaios-web/src/lib/kernel/orchestrator.ts`. Need a small integration fix so `websitefactory.lead_to_preview` loads/executes through plugin extension points.
