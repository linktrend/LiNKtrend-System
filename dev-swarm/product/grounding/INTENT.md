...
- **Vision:** [`VISION.md`](VISION.md) — product narrative (Planner fills after Chairman Go)
...
- Dev Swarm factory coordination for this repo (`dev-swarm/`)

### Out of scope

- Full live CRM, Plane, or hosted publishing without Chairman-approved capability live modes
- Customer-owned Plane/Odoo configuration beyond studio-provided MVO defaults
- New orchestration stacks (CrewAI, LangGraph, n8n brain) parallel to Dev Swarm
- Merging to `staging` or `main` without Chairman Release OK
...
- [ ] `verify.sh` passes at `DEV_SWARM_TIER=critical` for release scope
...
- [ ] Chairman Release OK before staging/main promotion
...
Intent verdict (G2): `dev-swarm/product/reports/linktrend-system/intent-verdict.json` — must be `PASS` before Orchestrator sets `swarm:ready`.
