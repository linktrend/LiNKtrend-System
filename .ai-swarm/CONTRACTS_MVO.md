...
### 0.A.5.1 Capability plugin contract pack v1 (connector-only)

Common contract shape for every LinkSites v1 capability plugin:

- `operations`: explicit operation ids only; no implicit actions.
- `modes`: `mock`, `shadow`, `live`; default execution mode for v2 MVO is `mock` unless stated otherwise.
- `auth_config_surface`: tenant config keys and secret references only; no credential literals in manifests.
- `idempotency`: required stable idempotency key shape and replay behavior.
- `linkskills_leases`: lease permissions/skills required before execution.
- `audit_events`: required LiNKbrain event types for request/start/success/failure.
- `allowed_callers`: `linkaios`, `vertical_plugin`, `linkbot`, `linkautowork` subset per capability.
- `failure_mapping`: connector/provider errors mapped to §5.4 canonical failure codes.
- `non_ownership`: explicit target-software internals this capability does not configure.

Per-capability v1 contracts:
...
|---|---|---|---|---|---|---|---|---|---|
| `cap.crm.odoo_shadow` | `lead.read_mock`, `lead.status.set_ready_to_contact`, `odoo.readiness.probe` | `mock` default for lead status writes, `shadow` for readiness probes, `live` disabled by default | `crm.provider`, `crm.mock_table_ref`, optional `odoo.base_url`, `odoo.credential_ref` | `(tenant_id, lead_id, target_status, run_id)` for status writes; readiness probes idempotent on `(tenant_id, probe_window)` | `crm.lead.read`, `crm.lead.status.write`, `crm.odoo.readiness.check` | `capability.requested`, `capability.executed`, `crm.lead.status.updated`, `crm.odoo.readiness.checked`, `capability.failed` | `vertical_plugin`, `linkautowork` | readiness/connectivity -> `INTEGRATION_UNAVAILABLE`; auth -> `INTEGRATION_AUTH_FAILED`; policy/kill switch -> `LEASE_DENIED`; invalid lease/input -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Odoo chart of accounts, accounting rules, CRM stage/taxonomy design, business master data |
| `cap.accounting.odoo_shadow` | `ledger.summary.read_mock`, `invoice.status.read_mock`, `odoo.readiness.probe` | `mock` default for local/read-only accounting surfaces, `shadow` for readiness probes, `live` disabled by default | `accounting.provider`, `accounting.mock_table_ref`, optional `odoo.base_url`, `odoo.credential_ref` | accounting reads idempotent on `(tenant_id, report_scope, as_of_date, run_id)`; readiness probes idempotent on `(tenant_id, probe_window)` | `accounting.ledger.read`, `accounting.invoice.read`, `accounting.odoo.readiness.check` | `capability.requested`, `capability.executed`, `accounting.ledger.summary.read`, `accounting.invoice.status.read`, `accounting.odoo.readiness.checked`, `capability.failed` | `vertical_plugin`, `linkautowork`, `linkaios` | readiness/connectivity -> `INTEGRATION_UNAVAILABLE`; auth -> `INTEGRATION_AUTH_FAILED`; policy/kill switch -> `LEASE_DENIED`; invalid lease/input -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Odoo chart of accounts design, journal/tax configuration, fiscal position rules, reconciliation policy, accounting master data |
| `cap.payload.local_sync` | `content.upsert_local`, `preview.publish_local`, `sync.status.read` | `mock` default (local Payload), `shadow` optional readiness only, `live` disabled by default | `payload.base_url`, `payload.space_ref`, `payload.credential_ref`, `payload.schema_source_ref` | `(tenant_id, site_id, site_generation_run_id, content_checksum)` | `payload.content.write`, `payload.preview.publish`, `payload.sync.read` | `capability.requested`, `capability.executed`, `payload.content.upserted`, `payload.preview.updated`, `payload.sync.checked`, `capability.failed` | `linkautowork`, `vertical_plugin` | missing schema mapping -> `MANIFEST_INVALID`; connectivity -> `INTEGRATION_UNAVAILABLE`; auth -> `INTEGRATION_AUTH_FAILED`; invalid lease/input -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Payload collection/schema design, field modeling, locale strategy, editorial workflow policy |
| `cap.supabase.mirror_content` | `site_content.upsert`, `asset_refs.upsert`, `mirror.status.read` | `mock` default (dev/local project), `shadow` optional, `live` disabled by default | `supabase.project_ref`, `supabase.schema_ref`, `supabase.credential_ref` | `(tenant_id, site_id, site_generation_run_id, payload_version)` | `supabase.mirror.write`, `supabase.mirror.read` | `capability.requested`, `capability.executed`, `supabase.mirror.content.upserted`, `supabase.mirror.asset_refs.upserted`, `capability.failed` | `linkautowork`, `vertical_plugin` | schema missing/not discovered -> `MANIFEST_INVALID`; RLS/auth -> `INTEGRATION_AUTH_FAILED`; connectivity -> `INTEGRATION_UNAVAILABLE`; invalid lease/input -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Supabase mirror table/column invention before WP-042 discovery, non-mirror business schema design |
| `cap.zulip.run_messaging` | `run.notify`, `channel.message.mock_send`, `connectivity.probe` | `mock` default for outbound messages, `shadow` for connectivity, `live` disabled by default | `zulip.base_url`, `zulip.bot_email_ref`, `zulip.api_key_ref`, `zulip.stream_ref`, `zulip.topic_template` | `(tenant_id, run_id, stage_id, message_purpose)` | `zulip.run.notify`, `zulip.channel.message.send` | `capability.requested`, `capability.executed`, `zulip.notification.queued`, `zulip.connectivity.checked`, `capability.failed` | `linkaios`, `vertical_plugin`, `linkbot`, `linkautowork` | stream/topic config missing -> `MANIFEST_INVALID`; auth -> `INTEGRATION_AUTH_FAILED`; connectivity -> `INTEGRATION_UNAVAILABLE`; policy disabled -> `LEASE_DENIED`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Zulip org stream taxonomy design beyond run-message needs, community/public broadcast policy |
| `cap.research.public_web` | `search.query`, `page.fetch`, `citation.extract` | `mock` (fixture/read cache) or `shadow` (real provider read-only), `live` same as shadow since no writes | `research.provider`, `research.api_key_ref`, `research.allow_domains`, `research.blocked_domains` | `(tenant_id, run_id, query_hash, provider)` | `research.public.read` | `capability.requested`, `capability.executed`, `research.query.performed`, `research.citation.recorded`, `capability.failed` | `linkbot`, `vertical_plugin`, `linkautowork` | provider quota/connectivity -> `INTEGRATION_UNAVAILABLE`; blocked-domain policy -> `LEASE_DENIED`; malformed query -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Target-site account setup, outreach actions, any write/submit side effect on external sites |
| `cap.asset.generation` | `image.generate`, `video.generate`, `asset.metadata.record` | `mock` default (placeholder/local generation), `shadow` optional provider readiness, `live` disabled by default | `asset.provider`, `asset.model_profile`, `asset.output_path_template`, `asset.credential_ref` | `(tenant_id, site_id, site_generation_run_id, asset_prompt_hash, asset_kind)` | `asset.generate.image`, `asset.generate.video`, `asset.metadata.write` | `capability.requested`, `capability.executed`, `asset.generated`, `asset.provenance.recorded`, `capability.failed` | `linkbot`, `vertical_plugin`, `linkautowork` | provider/connectivity -> `INTEGRATION_UNAVAILABLE`; provider auth -> `INTEGRATION_AUTH_FAILED`; moderation/policy block -> `LEASE_DENIED`; invalid lease/input -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Brand guideline authoring, DAM taxonomy design, external CDN publishing, production media rights policy |
| `cap.plane.execution_tracking` | `project.ensure_mock`, `task.ensure_mock`, `readiness.probe` | `mock` default for project/task writes, `shadow` for readiness, `live` disabled by default | `plane.base_url`, `plane.workspace_ref`, `plane.api_key_ref`, `plane.project_template_ref` | `(tenant_id, run_id, execution_scope, normalized_title)` | `plane.project.write`, `plane.task.write`, `plane.readiness.check` | `capability.requested`, `capability.executed`, `plane.project.upserted`, `plane.task.upserted`, `plane.readiness.checked`, `capability.failed` | `vertical_plugin`, `linkautowork`, `linkaios` | workspace/connectivity -> `INTEGRATION_UNAVAILABLE`; auth -> `INTEGRATION_AUTH_FAILED`; invalid lease/input -> `LEASE_REQUEST_INVALID`; policy disabled -> `LEASE_DENIED`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Plane workspace structure policy, sprint/workflow state taxonomy, client-facing project governance |
| `cap.postiz.distribution` | `connectivity.probe`, `draft.create_mock`, `schedule.mock`, `status.read` | `mock` default for draft/schedule surfaces, `shadow` only for connectivity/status checks, `live` disabled by default | `postiz.base_url`, `postiz.workspace_ref`, `postiz.credential_ref`, `postiz.channel_set_ref` | `(tenant_id, run_id, stage_id, operation, distribution_scope)` | `postiz.connectivity.check`, `postiz.draft.write`, `postiz.schedule.write`, `postiz.status.read` | `capability.requested`, `capability.executed`, `postiz.readiness.checked`, `postiz.draft.mocked`, `postiz.schedule.mocked`, `postiz.status.read`, `capability.failed` | `linkaios`, `vertical_plugin`, `linkbot`, `linkautowork` | auth -> `INTEGRATION_AUTH_FAILED`; connectivity -> `INTEGRATION_UNAVAILABLE`; policy disabled or publish-prohibited mode -> `LEASE_DENIED`; invalid lease/input -> `LEASE_REQUEST_INVALID`; timeout -> `INTEGRATION_TIMEOUT`; duplicate replay returns original result, conflict -> `LEASE_IDEMPOTENCY_CONFLICT` | Live post publish/send actions, account/channel/campaign setup, audience taxonomy design, marketing workflow ownership before Linktrend Media vertical definition |

Contract enforcement rules for v1 capability pack:

- All capabilities must default to `mock` or `shadow`; `live` requires explicit tenant opt-in plus granted lease.
- Every side-effecting operation must carry `lease_id`, `run_id`, and idempotency key.
- Connector contracts define communication/governance surface only and MUST NOT define business setup inside target software.
- CRM and Accounting capability IDs MUST remain permission-separated even when they share Odoo readiness/auth config.
- Postiz distribution capability MUST remain mock/shadow-safe until a Linktrend Media vertical plugin defines publish approvals and live routing.
