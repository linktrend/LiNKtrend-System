...
|---|---|---|---|---|---|---|---|---|---|
| `cap.crm.odoo_shadow` | `lead.read_mock`, `lead.status.set_ready_to_contact`, `odoo.readiness.probe` | `mock` default for lead status writes, `shadow` for readiness probes, `live` disabled by default | `crm.provider`, `crm.mock_table_ref`, optional `odoo.base_url`, `odoo.credential_ref` | `(tenant_id, lead_id, target_status, run_id)` for status writes; readiness probes idempotent on `(tenant_id, probe_window)` | `crm.lead.read`, `crm.lead.status.write`, `crm.odoo.readiness.check` | `capability.requested`, `capability.executed`, `crm.lead.status.updated`, `crm.odoo.readiness.checked`, `capability.failed` | `vertical_plugin`, `linkautowork` | readiness/auth failures -> `INTEGRATION_UNAVAILABLE`; policy/kill switch -> `LEASE_DENIED`; invalid status transition -> `INVALID_INPUT`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Odoo chart of accounts, accounting rules, CRM stage/taxonomy design, business master data |
| `cap.payload.local_sync` | `content.upsert_local`, `preview.publish_local`, `sync.status.read` | `mock` default (local Payload), `shadow` optional readiness only, `live` disabled by default | `payload.base_url`, `payload.space_ref`, `payload.credential_ref`, `payload.schema_source_ref` | `(tenant_id, site_id, site_generation_run_id, content_checksum)` | `payload.content.write`, `payload.preview.publish`, `payload.sync.read` | `capability.requested`, `capability.executed`, `payload.content.upserted`, `payload.preview.updated`, `payload.sync.checked`, `capability.failed` | `linkautowork`, `vertical_plugin` | missing schema mapping -> `PRECONDITION_FAILED`; connectivity/auth -> `INTEGRATION_UNAVAILABLE`; validation -> `INVALID_INPUT`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Payload collection/schema design, field modeling, locale strategy, editorial workflow policy |
| `cap.supabase.mirror_content` | `site_content.upsert`, `asset_refs.upsert`, `mirror.status.read` | `mock` default (dev/local project), `shadow` optional, `live` disabled by default | `supabase.project_ref`, `supabase.schema_ref`, `supabase.credential_ref` | `(tenant_id, site_id, site_generation_run_id, payload_version)` | `supabase.mirror.write`, `supabase.mirror.read` | `capability.requested`, `capability.executed`, `supabase.mirror.content.upserted`, `supabase.mirror.asset_refs.upserted`, `capability.failed` | `linkautowork`, `vertical_plugin` | schema missing/not discovered -> `PRECONDITION_FAILED`; RLS/auth/connectivity -> `INTEGRATION_UNAVAILABLE`; payload validation -> `INVALID_INPUT`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Supabase mirror table/column invention before WP-042 discovery, non-mirror business schema design |
| `cap.zulip.run_messaging` | `run.notify`, `channel.message.mock_send`, `connectivity.probe` | `mock` default for outbound messages, `shadow` for connectivity, `live` disabled by default | `zulip.base_url`, `zulip.bot_email_ref`, `zulip.api_key_ref`, `zulip.stream_ref`, `zulip.topic_template` | `(tenant_id, run_id, stage_id, message_purpose)` | `zulip.run.notify`, `zulip.channel.message.send` | `capability.requested`, `capability.executed`, `zulip.notification.queued`, `zulip.connectivity.checked`, `capability.failed` | `linkaios`, `vertical_plugin`, `linkbot`, `linkautowork` | stream/topic config missing -> `PRECONDITION_FAILED`; auth/connectivity -> `INTEGRATION_UNAVAILABLE`; policy disabled -> `LEASE_DENIED`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Zulip org stream taxonomy design beyond run-message needs, community/public broadcast policy |
| `cap.research.public_web` | `search.query`, `page.fetch`, `citation.extract` | `mock` (fixture/read cache) or `shadow` (real provider read-only), `live` same as shadow since no writes | `research.provider`, `research.api_key_ref`, `research.allow_domains`, `research.blocked_domains` | `(tenant_id, run_id, query_hash, provider)` | `research.public.read` | `capability.requested`, `capability.executed`, `research.query.performed`, `research.citation.recorded`, `capability.failed` | `linkbot`, `vertical_plugin`, `linkautowork` | provider quota/connectivity -> `INTEGRATION_UNAVAILABLE`; blocked-domain policy -> `POLICY_BLOCKED`; malformed query -> `INVALID_INPUT`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Target-site account setup, outreach actions, any write/submit side effect on external sites |
| `cap.asset.generation` | `image.generate`, `video.generate`, `asset.metadata.record` | `mock` default (placeholder/local generation), `shadow` optional provider readiness, `live` disabled by default | `asset.provider`, `asset.model_profile`, `asset.output_path_template`, `asset.credential_ref` | `(tenant_id, site_id, site_generation_run_id, asset_prompt_hash, asset_kind)` | `asset.generate.image`, `asset.generate.video`, `asset.metadata.write` | `capability.requested`, `capability.executed`, `asset.generated`, `asset.provenance.recorded`, `capability.failed` | `linkbot`, `vertical_plugin`, `linkautowork` | provider/connectivity -> `INTEGRATION_UNAVAILABLE`; moderation/policy block -> `POLICY_BLOCKED`; invalid prompt/input -> `INVALID_INPUT`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Brand guideline authoring, DAM taxonomy design, external CDN publishing, production media rights policy |
| `cap.plane.execution_tracking` | `project.ensure_mock`, `task.ensure_mock`, `readiness.probe` | `mock` default for project/task writes, `shadow` for readiness, `live` disabled by default | `plane.base_url`, `plane.workspace_ref`, `plane.api_key_ref`, `plane.project_template_ref` | `(tenant_id, run_id, execution_scope, normalized_title)` | `plane.project.write`, `plane.task.write`, `plane.readiness.check` | `capability.requested`, `capability.executed`, `plane.project.upserted`, `plane.task.upserted`, `plane.readiness.checked`, `capability.failed` | `vertical_plugin`, `linkautowork`, `linkaios` | workspace/auth/connectivity -> `INTEGRATION_UNAVAILABLE`; invalid mapping -> `INVALID_INPUT`; policy disabled -> `LEASE_DENIED`; timeout -> `TIMEOUT`; duplicate replay -> `IDEMPOTENT_REPLAY` | Plane workspace structure policy, sprint/workflow state taxonomy, client-facing project governance |
