...
| D-03 | 2026-05-14 | **Preview publishing:** static/local preview via `LiNKsites/apps/web-master` rendered + served from `apps/linkaios-web` for MVO; Payload/LinkSites full publish and Vercel deploy preview deferred | `LiNKsites/apps/web-master` exists and is Next.js. Static/local route produces a preview URL fast and keeps the demo deterministic. Payload CMS publish path stays available as the post-MVO upgrade. Vercel preview rejected for MVO due to per-deploy latency + token management cost. | LinkSites/Payload CMS path; static/local HTTP preview; Vercel deploy preview | **Accepted** (static/local) | linkaios-agent |
