...
describe("suite-role-openclaw barrel (Wave 1.8 / Wave 2)", () => {
  it("resolves LinkSites outreach to linksites-head (AZ roles excluded)", () => {
    expect(openClawAgentIdForRole("outreach_bot")).toBe("linksites-head");
    expect(openClawAgentIdForRole("research_enrichment_bot")).toBeNull();
    expect(openClawAgentIdForRole("librarian_bot")).toBeNull();
  });
  ...
  it("aggregates mappings from all suite barrels", () => {
    ...
    expect(mapped.has("outreach_bot")).toBe(true);
  });
