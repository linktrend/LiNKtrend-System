 ...
 *
 * Implements `bot.reason` per `LiNKdev/product/grounding/CONTRACTS_MVO.md` §6.1.
...
function generateStubResponse(userPrompt: string, systemPrompt: string): string {
  ...
  // LinkSites v2 MVO roles
  if (systemPrompt.includes("research enrichment")) {
    return JSON.stringify({
      lead_research_bundle: {
        market_context: "Local professional services market with moderate competition",
        audience_notes: "Small to medium business owners seeking reliable professional services",
        positioning_notes: "Trust, reliability, and years of expertise as key differentiators",
  ...
  if (systemPrompt.includes("website package generation")) {
    return JSON.stringify({
      website_package: {
        template_id: "professional_v1",
        sections: [
          { section_id: "hero", title: "Hero Section", blocks: ["hero_headline", "hero_subheadline", "hero_image"] },
          { section_id: "about", title: "About Us", blocks: ["about_lead", "about_image"] },
          { section_id: "services", title: "Services", blocks: ["services_intro", "services_icon"] },
          { section_id: "cta", title: "Call to Action", blocks: ["cta_primary"] },
        ],
        copy: {
          hero_headline: "Professional Services You Can Trust",
          hero_subheadline: "Serving your community with excellence",
          about_lead: "We bring years of expertise to every project.",
          services_intro: "Our services are tailored to your unique needs.",
          cta_primary: "Get Started Today",
        },
        media_plan: {
          hero_image: { asset_ref: "stock/hero-business-1", kind: "stock" },
          about_image: { asset_ref: "stock/about-team-1", kind: "stock" },
          services_icon: { asset_ref: "placeholder/service-icon", kind: "placeholder" },
        },
        provenance_notes: "Generated using professional_v1 template with market research context",
      },
    });
  }
  return "{\"result\": \"stub\"}";
...
Return ONLY valid JSON, no markdown formatting.`,
  ...
  research_enrichment: `You are a research enrichment assistant for the LinkSites WebsiteFactory module.
Analyze the provided lead context and return a JSON object with:
- lead_research_bundle: object containing market_context, audience_notes, and positioning_notes

Return ONLY valid JSON, no markdown formatting.`,

  website_package_generation: `You are a website package generation assistant for the LinkSites WebsiteFactory module.
Generate a preview-ready website package using the provided lead research and template guidance.

Return a JSON object with:
- website_package: object containing template_id, sections, copy, and provenance_notes
  ...
  try {
    ...
    switch (kind) {
      ...
      case "research_enrichment": {
        if (!parsed.lead_research_bundle) {
          return {
            code: "MODEL_OUTPUT_INVALID",
            plane: "linkbot",
            message: "research_enrichment output missing required field (lead_research_bundle)",
            retryable: false,
            occurred_at: new Date().toISOString(),
          };
        }
        return { lead_research_bundle: parsed.lead_research_bundle };
      }

      case "website_package_generation": {
        if (!parsed.website_package) {
          return {
            code: "MODEL_OUTPUT_INVALID",
            plane: "linkbot",
            message: "website_package_generation output missing required field (website_package)",
            retryable: false,
            occurred_at: new Date().toISOString(),
          };
        }
        return { website_package: parsed.website_package };
      }
    }
  } catch (e) {
    return {
...
function roleForReasoning(kind: ReasoningKind): "research_enrichment_bot" | "website_builder_bot" {
  // Per CONTRACTS_MVO.md §0.A.4 - LinkSites v2 roles
  switch (kind) {
    case "lead_evaluation":
    case "research_enrichment":
    ...
    default:
      // Default to website_builder_bot for unknown kinds (safer for generation tasks)
      return "website_builder_bot";
  }
}
