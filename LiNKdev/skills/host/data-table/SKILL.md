...
9. **Title Case** column headers via `TABLE_COLUMN` or `formatTableColumnLabel()`. Section headings and form labels use the same Title Case rule via `formatUiLabel()` — see `.cursor/rules/08-linkaios-ui-standards.mdc`.
...
See `LiNKaios/linkaios-web/src/components/data-table/README.md` and `.cursor/rules/08-linkaios-ui-standards.mdc`.
