...
**User notes:** Principal prefers All Work action queue over coloured icon boxes; Overview updated to match.
...
**User notes:** Principal agrees with recommendation; mock is fine for current review phase.
...
**Problem:** Principal needs to preview how the app will look and work in real life; some sections still feel empty or thin.
...
Principal approved items 1–19 with inbox-first workflow preserved (Add Knowledge → Inbox → approve → LiNKbrain).
...
Principal approved items 1–17. Implemented in UI review pass (no commit unless requested).
...
**Ground truth:** Principal terminology (Project Type = pre-defined Process); orchestrator nav spec; `modules/module-registry.md`; `modules/linksites/workflow.md` (10-stage spine).
...
| ID | Title | Notes |
|----|-------|-------|
| UIUX-MOD-H001 | Full Modules IA redesign per orchestrator spec | Principal flagged substantial redo |
...
**Principal intent:** Operators observe performance as granularly as possible — per project, project type (process), workflow, issue, LiNKbot, automation — across tokens, cost, run time, success/failure, errors, tool/skill/model usage.
...
**Principal decisions (locked):** Licensee company hub; LiNKbrain Inbox for uploads; locations + org structure; users = Settings; module subscribe/cancel (full flow, billing stubbed for MVP); multi-company + multi-membership; industry picklist; AI agent users are vendor-side; hybrid IA (Company sidebar + Settings admin).
...
Agent reports: `LiNKdev/product/reports/archive/legacy-ai-swarm/wp-wave1-*.md`, `wp-wave2-*.md`
...
**Remaining:** see `LiNKdev/product/grounding/TODO-WAVE3-REMAINING.md`
