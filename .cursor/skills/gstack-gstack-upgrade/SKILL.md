...
```bash
cd "$_ROOT"
git rm -r --cached ../gstack/ 2>/dev/null || true
if ! grep -qF '../gstack/' .gitignore 2>/dev/null; then
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `../gstack/` when you're ready."
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `../gstack/` when you're ready."
