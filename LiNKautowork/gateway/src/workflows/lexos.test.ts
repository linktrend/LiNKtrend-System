...
describe("LEXOS Workflow Handlers", () => {
  ...
  describe("autowork.lexos.evidence_ingest", () => {
    ...
    it("should succeed with valid inputs and lease_id", async () => {
      ...
      if ("outputs" in result) {
        expect(result.outputs.evidence_ids).toHaveLength(2);
        ...
        const evidence0 = getEvidence(result.outputs.evidence_ids[0] as string);
    ...
    it("should mark scanned documents for human review", async () => {
      ...
      if ("outputs" in result) {
        ...
        expect(result.outputs.extractions[0].human_review_required).toBe(true);
        expect(result.outputs.extractions[0].extraction_quality_status).toBe("human_review_required");
      }
    });
  ...
  describe("autowork.lexos.assertion_sync", () => {
    ...
    it("should mark assertions as supported when evidence provided", async () => {
      ...
      expect("outputs" in result).toBe(true);
      if ("outputs" in result) {
        expect(result.outputs.assertion_updates[0].support_state).toBe("supported");
        expect(result.outputs.assertion_updates[0].supporting_evidence).toContain(evidenceId);
        expect(result.outputs.assertion_updates[0].confidence).toBe(0.85);
      }
    });
  ...
  describe("autowork.lexos.artifact_generate", () => {
    ...
    it("should succeed with valid inputs and lease_id", async () => {
      ...
      if ("outputs" in result) {
        ...
        expect(result.outputs.caveat_preservation_check.caveats_preserved).toBe(true);
        expect(result.outputs.lease_id).toBe(request.lease_id);
