import { embedMissingSkillSlimIndexes } from "@linktrend/linklogic-sdk";
...
/**
 * Async skill slim embedding worker. Auth: same as brain-embed cron.
 */
async function handleSkillEmbed(req: Request) {
  ...
  const result = await embedMissingSkillSlimIndexes(supabase, { apiKey: key, limit });
  return NextResponse.json({ ok: true, ...result });
}

export async function POST(req: Request) {
  return handleSkillEmbed(req);
}
