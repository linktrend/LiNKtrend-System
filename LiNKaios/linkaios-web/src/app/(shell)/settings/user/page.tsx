...
import { getAppRoleTierForUser, getCommandCentreRoleForUser } from "@/lib/command-centre-access";
...
export default async function SettingsUserPage() {
  ...
  const appRoleTier = await getAppRoleTierForUser(supabase, { userId: user.id, email: user.email });
  ...
  const accessScope = resolveOperatorAccessScope({ email, role, surface, appRoleTier });
