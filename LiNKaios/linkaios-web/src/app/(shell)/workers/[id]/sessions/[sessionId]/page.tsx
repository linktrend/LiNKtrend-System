...
import { sessionDetailFromMetadata } from "@/lib/session-detail-data";
...
function SessionDetailShell(props: { agentId: string; row: SessionThreadRow; focusTranscript?: boolean }) {
  ...
  const { transcript, toolCalls } = sessionDetailFromMetadata(row.metadata);
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="border-b border-zinc-200 pb-6 dark:border-zinc-800">
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{row.sessionTitle}</h1>
        <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">Agent</dt>
            <dd className="mt-1 text-zinc-900 dark:text-zinc-100">{row.agentName}</dd>
          </div>
          <div>
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <ol className="mt-4 max-h-56 space-y-3 overflow-y-auto border-l border-zinc-200 pl-4 dark:border-zinc-700">
          {timeline.map((step, i) => (
      ...
      >
        ...
        {transcript.length > 0 ? (
          <ul className="mt-4 max-h-80 space-y-3 overflow-y-auto rounded-lg border border-zinc-100 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900/60">
            {transcript.map((entry, index) => (
              <li key={index} className="text-sm">
                <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">{entry.role}</span>
          ...
          <>
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              No structured transcript yet — showing session metadata when available.
            </p>
      ...
      <SessionInteractionPanel toolCalls={toolCalls} />
    </div>
  );
}
