...
import { ModuleOwnedCard } from "@/components/suites/module-owned-card";
...
import { modulesForCompany } from "@/lib/company-fixtures";
import { appendLicenseeContext } from "@/lib/licensee-context";
import { suiteEntitlementForModule } from "@/lib/suite-entitlements";
...
export function ModulesMyModulesPanel() {
  const { companyId, effectiveBrandId } = useLicenseeContext();
  ...
  const companyModuleIds = useMemo(() => {
    const rows = modulesForCompany(companyId);
    return new Set(
      rows.filter((r) => r.status === "active" || r.status === "trialing").map((r) => r.moduleId),
    );
  }, [companyId]);
  ...
  const owned = MODULES_CATALOG_DEMO.modules
    .filter((m) => {
      ...
      const entitled = companyModuleIds.has(m.id);
  ...
  const marketplaceHref = appendLicenseeContext("/suites/marketplace", {
    companyId,
  ...
  if (owned.length === 0) {
    return (
      <p className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50/80 p-6 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900/40 dark:text-zinc-400">
        No suites for this company yet.{" "}
        <Link href={marketplaceHref} className="font-medium text-zinc-800 underline-offset-2 hover:underline dark:text-zinc-200">
          Browse marketplace
        </Link>
      </p>
    );
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-zinc-600 dark:text-zinc-400">
        Suites entitled for the active company. Billing scope follows each suite&apos;s entitlement model (
        {owned.map((m) => suiteEntitlementForModule(m.id).scope).filter((v, i, a) => a.indexOf(v) === i).join(", ")}).
      </p>
