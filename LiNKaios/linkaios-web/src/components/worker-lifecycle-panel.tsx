...
import { BUTTON, formatUiLabel, TYPE, WORKER_DETAIL } from "@/lib/ui-standards";
...
export function WorkerLifecyclePanel(props: { agentId: string; displayName: string }) {
  ...
  if (!canManage) {
    return (
      <p className={WORKER_DETAIL.notice}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        <span className={WORKER_DETAIL.bodyMuted}>Current state</span>
        <StatusPill
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        <h3 className={WORKER_DETAIL.cardTitle}>Suspend</h3>
        <p className={`mt-1 ${WORKER_DETAIL.bodyMuted}`}>
      ...
      <section className="rounded-xl border border-amber-200 bg-amber-50/60 p-5 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/20">
        <h3 className={`${WORKER_DETAIL.cardTitle} text-amber-950 dark:text-amber-50`}>Terminate</h3>
        <p className={`mt-1 ${TYPE.body} text-amber-900/90 dark:text-amber-100/90`}>
      ...
      <section className="rounded-xl border border-red-200 bg-red-50/50 p-5 shadow-sm dark:border-red-900/40 dark:bg-red-950/20">
        <h3 className={`${WORKER_DETAIL.cardTitle} text-red-950 dark:text-red-100`}>Delete</h3>
        <p className={`mt-1 ${TYPE.body} text-red-900/90 dark:text-red-100/90`}>
          Request permanent removal of {props.displayName} from the fleet registry. Deletion runs only after governance
          approval.
        </p>
        <label className="mt-3 block space-y-1.5">
          <span className={`${WORKER_DETAIL.dlLabel} text-red-900 dark:text-red-200`}>Type DELETE to confirm</span>
          <input
            value={confirmDelete}
