    ...
    <div className="grid gap-6 md:grid-cols-2">
      <article className="flex flex-col rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Packaged procedures that call governed tools and capability connectors at runtime.
        </p>
      ...
      <article className="flex flex-col rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Callable integrations and actions executed through approved capability governance.
        </p>
        <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
          <span className="rounded-full bg-emerald-50 px-2 py-0.5 font-medium text-emerald-900 ring-1 ring-emerald-300 dark:bg-emerald-950/30 dark:text-emerald-200 dark:ring-emerald-700">
            Output: produced artifact/data
          </span>
          <span className="rounded-full bg-amber-50 px-2 py-0.5 font-medium text-amber-900 ring-1 ring-amber-300 dark:bg-amber-950/30 dark:text-amber-200 dark:ring-amber-700">
      ...
      <article className="flex flex-col rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        <h2 className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Capability connectors</h2>
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Governed bridges to external software — Odoo, Plane, Payload, Zulip, and the connector registry.
        </p>
        <div className="mt-4 flex-1">
          <StatLine label="Registered connectors" value={props.connectors.total} />
          <StatLine label="Implemented" value={props.connectors.implemented} />
          <StatLine label="Declared" value={props.connectors.declared} />
          <StatLine label="Pending" value={props.connectors.pending} />
        </div>
        <Link href="/skills/connectors" className={`${BUTTON.secondaryCardAction} mt-6`}>
          Open Connectors catalogue
        </Link>
      </article>

      <article className="flex flex-col rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
        <h2 className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Leases</h2>
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Time-scoped grants for capabilities, tools, and side effects — plus kill switches and approval posture.
        </p>
        <p className="mt-3 flex-1 text-xs text-zinc-600 dark:text-zinc-400">
          Runtime execution always flows through LinkSkills leases and audit — even when catalogue rows are enabled.
        </p>
        <Link href="/skills/leases" className={`${BUTTON.secondaryCardAction} mt-6`}>
          Open leases
        </Link>
      </article>
    </div>
