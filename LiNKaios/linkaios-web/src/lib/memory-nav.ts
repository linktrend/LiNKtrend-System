...
export const LINKBRAIN_NAV_TABS: { id: LinkbrainTab; label: string }[] = [
...
function memorySubRouteTab(path: string): LinkbrainTab | null {
  ...
  if (path.startsWith("/memory/company-structure")) return "orgScope";
...
export function matchLinkbrainTab(id: LinkbrainTab, path: string, search?: string): boolean {
  if (path.startsWith("/memory/") && !isMemoryHubPath(path)) {
    const subTab = memorySubRouteTab(path);
    return subTab != null && subTab === id;
  }
  if (!isMemoryHubPath(path)) return false;
