...
import { buildLinktrendClientFleet, linktrendStudioModuleEntitlements } from "@/lib/kernel/fleet/client-tenant-linktrend";
...
describe("linktrend client tenant (Wave 7)", () => {
  it("provisions ceo-client and subscribes linksites + linkdeveloper", () => {
    const fleet = buildLinktrendClientFleet();
    expect(fleet.provision.bindings[0]?.openclawAgentId).toBe("ceo-client");
    expect(fleet.subscribed).toEqual(["linksites", "linkdeveloper"]);
    expect(linktrendStudioModuleEntitlements()).toContain("linkdeveloper");
  });
});
